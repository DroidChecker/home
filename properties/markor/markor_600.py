...
class Test(AndroidCheck):
    ...
    @precondition(
        ...
        self.device(resourceId="net.gsantner.markor:id/action_go_to").exists() and not 
        self.device(text="Settings").exists() and not self.device(text="Date").exists() and not 
        self.device(resourceId="net.gsantner.markor:id/action_rename_selected_item").exists()
        )
    @rule()
    def markor_title_disappear(self):
        assert not self.device(resourceId="net.gsantner.markor:id/action_go_to").left(className="android.widget.TextView") is None
